...
import {
  Paper, Avatar, Container, Table, TableBody, TableFooter, TablePagination, TableRow, Typography, CircularProgress, Chip, makeStyles, TableCell
...
const Users: React.FC = () => {
  ...
  const selectRow = (a:any, b:any) => {

  }

  const handleChangePageUser = (e: any, page: number) => {
    dispatch(actions.usersListChangePage(page))
  }
  ...
  return (
    <Paper>
          <TableTitle
            ...
            // }}
            // addRow={() => props.history.push('contractor-form-user-new')}
          />
            ...
            <TableBody>
              {users.rows.map((user, index) => (
                <TableRow
                  ...
                  key={user.id}
                  hover
                  onClick={event => selectRow(event, user.id)}
								>
